...
create table if not exists transportadora_x_contato(
	/*trs_x_cnt_id int auto_increment,*/
	trs_id int,
    cnt_id int,
    
    /*constraint pk_transportadora_x_contato primary key(trs_x_cnt_id),*/
...
create table if not exists item_entrada(
    ...
    constraint fk_i_e_nota_fiscal foreign key(nf_id) references nota_fiscal(nf_id)
);
...
create table if not exists item_saida(
    ...
    constraint fk_i_s_nf_id foreign key(nf_id) references nota_fiscal(nf_id)
);