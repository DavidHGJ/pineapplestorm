...
export default {
  ...
  methods: {
    carregarContatos() {
      api
        .get(`/filial-contato/${this.$route.params.id}`)
        .then((res) => {
    ...
    postContato() {
      api
        .post("/filial-contato", this.editedItem)
        .then(() => {
    ...
    deleteContato(id) {
      api
        .delete(`/filial-contato/${this.$route.params.id}`, {
          data: { CNT_ID: id },
        })
        .then(() => {
    ...
    validaCampos() {
      ...
      this.editedItem.FIL_ID = this.$route.params.id;
